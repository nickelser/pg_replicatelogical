...
const usage string = `pg_replicatelogical: replicate output from Postgres' test_decoder plugin

Usage:
  pg_replicatelogical --source=postgres://user:pass@src.example.com/sourcedb --target=postgres://user:pass@tgt.example.com/targetdb --tables=public.users,public.events

Options:
  -h --help     Show this message.
  --slot        Logical replication slot on the source DB. (required)
  --source      Source Postgres database URI/DSN. (required)
  --target      Target Postgres database URI/DSN. (required)
  --create      Create the specified replication slot. Run this once per desired slot; exits upon success. --tables ignored.
  --drop        Drop the specified replication slot. Run this once per slot to remove; exits upon success. --tables ignored.
  --tables      Table(s) to transfer. Defaults to all tables. Specify the schema as well as the table name (e.g., --tables=public.users).
`
